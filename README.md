...
:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our free [CData MCP Server for Azure Active Directory (beta)](https://www.cdata.com/download/download.aspx?sku=CJZM-V&type=beta). 
## Purpose
We created this read-only MCP Server to allow LLMs (like Claude Desktop) to query live data Azure Active Directory supported by the [CData JDBC Driver for Azure Active Directory](https://www.cdata.com/drivers/azureactivedirectory/jdbc).
...
1. Clone the repository:
      ```bash
      git clone https://github.com/cdatasoftware/azure-active-directory-mcp-server-by-cdata.git
      cd azure-active-directory-mcp-server-by-cdata
      ```
2. Build the server:
      ```bash
      mvn clean install
...
2. Download and install the CData JDBC Driver for {source}: [https://www.cdata.com/drivers/azureactivedirectory/download/jdbc](https://www.cdata.com/drivers/azureactivedirectory/download/jdbc)
3. License the CData JDBC Driver:
    * Navigate to the `lib` folder in the installation directory, typically:
        * (Windows) `C:\Program Files\CData\CData JDBC Driver for Azure Active Directory\`
        * (Mac/Linux) `/Applications/CData JDBC Driver for Azure Active Directory/`
    * Run the command `java -jar cdata.jdbc.azuread.jar --license`
    * Enter your name, email, and "TRIAL" (or your license key).
4. Configure your connection to the data source (Salesforce as an example):
    * Run the command `java -jar cdata.jdbc.azuread.jar` to open the Connection String utility.
...
5. Create a `.prp` file for your JDBC connection (e.g. `azure-active-directory.prp`) using the following properties and format:
    ...
    * **Tables** - leave blank to access all data, otherwise you can explicitly declare the tables you wish to create access for
      ```env
      Prefix=azuread
      ServerName=CDataAzureAD
      ServerVersion=1.0
      DriverPath=PATH\TO\cdata.jdbc.azuread.jar
      DriverClass=cdata.jdbc.azuread.AzureADDriver
      JdbcUrl=jdbc:azuread:InitiateOAuth=GETANDREFRESH;
      Tables=
      ```
...
```json
{
    ...
    "method": "tools/call",
    "params": {
        "name": "azure_active_directory_get_tables",
        "arguments": {}
    }
}
```
...
```json
{
    ...
    "method": "tools/call",
    "params": {
        "name": "azure_active_directory_get_columns",
        "arguments": {
            "table":  "Account"
        }
    }
...
```json
{
    ...
    "method": "tools/call",
    "params": {
        "name": "azure_active_directory_run_query",
        "arguments": {
